        ...
        if self.verbose:
            ...
            if mask_index is not None:
                ...
                # _preds = torch.cat([torch.index_select(a, 0, i[i!=-1]).unsqueeze(0) for a, i in zip(A, mask_index)])
                    ...
                    print('preds_max:', torch.max(_preds,1)[0])
...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None):
        ...
        if labels is not None:
            ...
            # print('loss : ', loss.item(), ' logits: ', logits)
            # probs = logits.view(-1,self.num_labels).gather(1, labels)

            writer.add_scalar('classification_loss', loss.item(), self.bert.iteration)
            # writer.add_scalar('Good class prob', probs.mean().item(), self.bert.iteration)
            return loss
        else:
            return logits
...
class BertForSentiment(BertPreTrainedModel):
    ...
    Inputs:
        ...
        `attention_mask`: an optional torch.LongTensor of shape [batch_size, num_choices, sequence_length] with indices
            ...
            a batch has varying length sentences.
        `labels`: labels for the classification output: torch.LongTensor of shape [batch_size]
            with indices selected in [0, ..., num_choices].1
    ...
    ```
    """
    def __init__(self, bert, num_choices=2, dropout = 0.1, hidden_size = 768):
